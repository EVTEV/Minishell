...
/*   Updated: 2025/03/14 18:06:51 by lowatell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ast	*handle_pipe(char **token, int i, t_ast *root, t_data *data)
{
	...
	node = create_node(PIPE, NULL, root, data);
	if (!node)
		return (NULL);
...
t_ast	*handle_redirect(char **token, int *i, t_ast *root, t_data * data)
{
	if (!token[*i + 1])
		return (NULL);
	return (create_node(REDIR, token[++(*i)], root, data));
}
...
int	add_arg_to_cmd(t_data *data, t_ast **root, char *arg)
{
	if (!*root)
	{
		*root = create_node(CMD, NULL, NULL, data);
		return (0);
	}
...
t_ast	*pars_command(char **token, t_data *data)
{
	...
	while (token[i])
	{
		if (ft_strncmp(token[i], "|", 1) == 0)
			...
			root = handle_redirect(token, &i, root, data);
